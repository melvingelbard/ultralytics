...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45, class_category="", task='detect'):
    ...
    def plot(self, normalize=True, save_dir='', names=(), on_plot=None):
        ...
        title = 'Confusion Matrix (' + self.class_category + ')' + ' Normalized' * normalize
        ax.set_xlabel('True')
...
class ClassifyMetrics(SimpleClass):
    ...
    Properties:
        fitness (float): The fitness of the model.
    ...
    def process(self, targets, pred):
        ...
        pred, targets = torch.cat(pred)[:, 0], torch.cat(targets)
        self.accuracy = sklearn_metrics.accuracy_score(targets.cpu(), pred.cpu())
        self.precision = sklearn_metrics.precision_score(targets.cpu(), pred.cpu(), zero_division=1.0, average="macro")
        self.recall = sklearn_metrics.recall_score(targets.cpu(), pred.cpu(), zero_division=1.0, average="macro")
    ...
    def results_dict(self):
        ...
        return dict(zip(self.keys + [self.task + '-fitness'], [self.precision, self.recall, self.accuracy, self.fitness]))

    @property
    def keys(self):
        """Returns a list of keys for the results_dict property."""
        metrics = ['metrics/precision', 'metrics/recall', 'metrics/accuracy', 'metrics/fitness']
        return ["{}-{}".format(self.task, metric) for metric in metrics]
