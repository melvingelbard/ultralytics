...
def check_cls_age_gender_dataset(dataset: str, split=''):
    """
    Checks an age and gender classification dataset.
    ...
    val_set = data_dir / 'valid.csv' if (data_dir / 'valid.csv').exists() else None  # data/test or data/valid
    test_set = data_dir / 'test' if (data_dir / 'test').exists() else None  # data/val or data/test
    if split == 'val' and not val_set:
        LOGGER.info("WARNING ⚠️ Dataset 'split=val' not found, using 'split=test' instead.")
    elif split == 'test' and not test_set:
    ...
    nc_age = len(df['age'].unique())
    nc_gender = len(df['gender'].unique())
    names_age = {0: "Child", 1: "Teen", 2: "Adult", 3: "Senior", 4: "Unknown"}
    names_gender = {0: "Male", 1: "Female", 2: "Unknown"}
    return {'train': train_set, 'val': val_set or test_set, 'test': test_set or val_set, 'nc_age': nc_age, 'nc_gender': nc_gender, 'names_age': names_age, 'names_gender': names_gender}
