...
from .utils import HELP_URL, LOGGER, get_hash, img2label_paths, verify_image_label, verify_image, IMAGENET_MEAN, \
    IMAGENET_STD
...
class AgeGenderClassificationDataset(Dataset):
    ...
    def read_samples(self):
        self.samples = []
        sample_abspath = str(self.root)[:-4] # Up to '..train', without the '.csv'
        with open(sample_abspath + ".csv", newline='') as csvfile:
            samples_csv = csv.reader(csvfile, delimiter=',')
            next(samples_csv, None)
            for sample_csv in samples_csv:
                sample = (os.path.join(sample_abspath, sample_csv[0]), tuple((sample_csv[1], sample_csv[2])))
