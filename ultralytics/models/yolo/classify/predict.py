...
class AgeGenderClassificationPredictor(BasePredictor):
    ...
    def preprocess(self, img):
        """Converts input image to model-compatible data type."""
        self.transforms = classify_transforms(self.args.imgsz)
        if not isinstance(img, torch.Tensor):
            img = torch.stack([self.transforms(im) for im in img], dim=0)
        img = (img if isinstance(img, torch.Tensor) else torch.from_numpy(img)).to(self.model.device)
        return img.half() if self.model.fp16 else img.float()  # uint8 to fp16/32
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, pred in enumerate(preds[0]):
            ...
            path = self.batch[0]
            img_path = path[i] if isinstance(path, list) else path
            results.append(Results(orig_img=orig_img, path=img_path, names=self.names_age, probs=pred))

        for i, pred in enumerate(preds[1]):
            orig_img = orig_imgs[i] if isinstance(orig_imgs, list) else orig_imgs
            path = self.batch[0]
            img_path = path[i] if isinstance(path, list) else path
            results.append(Results(orig_img=orig_img, path=img_path, names=self.names_gender, probs=pred))
